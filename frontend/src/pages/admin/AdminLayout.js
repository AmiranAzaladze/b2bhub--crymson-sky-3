...
export default function AdminLayout() {
  ...
  return (
    <CountriesContext.Provider value={{ countries, refresh }}>
      <div className="min-h-screen bg-[#FAFAFA] flex" data-testid="admin-shell">
        {/* Sidebar */}
        <aside className="w-72 shrink-0 bg-white border-r border-neutral-200 flex flex-col h-screen sticky top-0">
          <div className="px-5 py-5 border-b border-neutral-200">
            <NavLink to="/admin" className="flex items-center gap-2" data-testid="admin-brand">
              <div className="h-7 w-7 rounded-md bg-[#0A0A0A] grid place-items-center">
                <span className="text-white font-display font-bold text-[13px]">SF</span>
              </div>
              <div>
                <div className="font-display font-bold text-[15px] leading-none tracking-tight">
                  Swift Formations
                </div>
                <div className="font-mono text-[9px] uppercase tracking-[0.22em] text-neutral-500 mt-1">
                  Admin · Multi-tenant
                </div>
              </div>
            </NavLink>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-4">
            <div className="flex items-center justify-between px-2 mb-3">
              ...
              <button
                ...
                className="h-6 w-6 grid place-items-center rounded-md hover:bg-neutral-100 text-neutral-600"
            ...
            {loading ? (
              ...
              <nav className="space-y-0.5">
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-2 px-2.5 py-1.5 rounded-md text-[13px] font-medium ${
                ...
                {countries.map((c) => (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `group flex items-center gap-2 px-2.5 py-2 rounded-md text-[13px] ${
                        isActive
                  ...
                  >
                    ...
                    <span
                      className={`ml-auto h-1.5 w-1.5 rounded-full shrink-0 ${
                        c.status === "published" ? "bg-green-500" : "bg-neutral-300"
                      }`}
                      title={c.status}
                    />
                    <ChevronRight className="h-3 w-3 text-neutral-400 opacity-0 group-hover:opacity-100" />
                  </NavLink>
                ))}
              </nav>
            )}
          ...
          <div className="px-3 py-3 border-t border-neutral-200">
            <div className="flex items-center justify-between px-2 py-1.5">
              <div className="min-w-0">
                <div className="text-[12.5px] font-medium text-neutral-950 truncate">
                  {user?.email}
                </div>
                <div className="font-mono text-[9px] uppercase tracking-[0.22em] text-neutral-500 mt-0.5">
                  {user?.role}
                </div>
              </div>
              <button
                onClick={onLogout}
                className="h-7 w-7 grid place-items-center rounded-md hover:bg-neutral-100 text-neutral-600"
...
function AddCountryDialog({ open, onOpenChange, onCreated }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[560px] p-0 overflow-hidden rounded-2xl" data-testid="add-country-dialog">
        ...
        <form onSubmit={submit} noValidate className="px-7 pb-7 pt-3 space-y-3" data-testid="add-country-form">
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Name (short)" id="name">
          ...
          <Field label="Brand name" id="brand_name">
          ...
          <div className="grid grid-cols-3 gap-3">
            <Field label="Flag emoji" id="flag">
          ...
          <Button
            type="submit"
            disabled={loading}
            className="w-full h-11 bg-[#0A0A0A] hover:bg-neutral-800 text-white rounded-full text-[13.5px] font-medium mt-2"
...
const Field = ({ label, id, children }) => (
  <div>
    <Label htmlFor={id} className="text-[12px] font-medium text-neutral-700 mb-1.5 block">{label}</Label>
    {children}
  </div>
);
